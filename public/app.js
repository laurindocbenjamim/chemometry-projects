...
function updateFileBadges() {
    ...
    if (uploadedFiles.length === 0) {
        ...
        const csvCount = uploadedFiles.filter(f => f.name.toLowerCase().endsWith('.csv')).length;
        const spCount = uploadedFiles.filter(f => f.name.toLowerCase().endsWith('.sp')).length;
        ...
        if (csvCount > 0) formatBreakdown.push(`${csvCount} CSV`);
        if (spCount > 0) formatBreakdown.push(`${spCount} SP`);
        const breakdownText = formatBreakdown.length > 0 ? ` (${formatBreakdown.join(', ')})` : '';
        fileCountEl.textContent = `Total Files Selected: ${uploadedFiles.length}${breakdownText} - ${sizeText}`;
    }
...
function setupVisualizationSwitchers() {
    ...
    btnDownloadAll.addEventListener("click", (e) => {
        ...
        document.getElementById("export-pdf-btn").addEventListener("click", () => {
            ...
            const reqTitle = document.getElementById("report-title")?.value || "AI Scientific Spectroscopy Diagnostics Report";
            const reqSubtitle = document.getElementById("report-subtitle")?.value || "AUTOGENERATED SCIENCE PIPELINE REPORT";
    ...
    btnReset.addEventListener("click", () => {
        ...
        document.getElementById("pdf-report-settings-container").style.display = "none";
        btnDownloadAll.style.display = "none";
...
function renderDashboard(data) {
    ...
    document.getElementById("pdf-report-settings-container").style.display = "flex";
